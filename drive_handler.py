    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'SERVICE_DISABLED' in str(e):
                ...
                print("1. Go to: https://console.developers.google.com/apis/api/sheets.googleapis.com")
...
def find_spreadsheet(drive_service, file_name, folder_id):
    """Find existing spreadsheet by exact name in a specific folder"""
...
def upload_csv_to_drive(drive_service, sheets_service, file_path, folder_id):
    ...
    if not sheets_service:
        ...
        print("Note: This will not preserve any formatting or formulas in existing sheets")
        ...
        try:
            ...
            print(f"Successfully created new spreadsheet: {file['name']} (ID: {file['id']})")
            return file['id']
        except Exception as e:
    ...
    if existing_file:
        try:
            ...
            if 'values' in sheet_data and len(sheet_data['values']) > 1:
                ...
                if 'campaign_id' in existing_df.columns and 'campaign_id' in new_data_df.columns:
                    existing_df['unique_key'] = existing_df['date'].astype(str) + '_' + existing_df['campaign_id'].astype(str)
                    new_data_df['unique_key'] = new_data_df['date'].astype(str) + '_' + new_data_df['campaign_id'].astype(str)
                    
                    # Remove rows from existing data that will be updated
                    merged_df = pd.concat([
                        existing_df[~existing_df['unique_key'].isin(new_data_df['unique_key'])],
                        new_data_df
                    ])
                    ...
                    if 'date' in merged_df.columns and 'campaign_id' in merged_df.columns:
                    ...
                    for _, row in merged_df.iterrows():
                        ...
                        cleaned_row = [str(val) if val != '' else '' for val in row.tolist()]
                    ...
                    print(f"Updated {response.get('updatedCells')} cells in the spreadsheet")
